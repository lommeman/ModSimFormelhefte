...
where:
\begin{itemize}
    \item \( \dot{\omega}_i \) is the magnitude of the time derivative of the angular velocity component \( \omega_i \).
    \item \( \vec{e}_i \) is the axis of rotation for component i.
    \item \( {\omega}_i \) is the magnitude of its rate of rotation. 
    \item \( \vec{\Omega}_i \) is the angular velocity of the reference frame where the vector \( \vec{e}_i \) is fixed (i.e has constant coordinates).
